...
struct ping_list
{
    char pingip[INET_ADDRSTRLEN];
    char mac_addr[6];
    int nsent;
    
}pinglist[10];  //ping list to check wether previous node has been already processed
int pingnodecount;
...
int areq (struct sockaddr *IPaddr, socklen_t sockaddrlen, struct hwaddr *HWaddr)
{
    ...
    {
        printf("\n Error in creation of Unix socket in areq. Check if areq is running \n ");
    }
...
void process_pingreply(char *ptr, ssize_t len, struct timeval *tvrecv,struct sockaddr_in pgaddr)
{
    int     hlenl, icmplen;
    double  rtt;
    struct ip *ip;
    ...
    if (icmp->icmp_type == ICMP_ECHOREPLY) {
        ...
        printf ("%d bytes from %s: seq=%u, ttl=%d, rtt=%.3f ms\n",
                icmplen,inet_ntop(AF_INET,&(pgaddr.sin_addr),paddr,INET_ADDRSTRLEN),
...
void update_pinglist(char IP[INET_ADDRSTRLEN])
{
    strcpy(pinglist[pingnodecount].pingip,IP);
    strcpy(pinglist[pingnodecount].mac_addr,HWaddr.sll_addr);
    pingnodecount++;
...
void echo_request()
{
    ...
    for(count=0;count<pingnodecount;count++) // Pinging over entire ping list
    {
        
        printf("PING %s (%s): %d data bytes\n\n", pinglist[count].pingip, pinglist[count].pingip, DATALEN);
        // Set destination MAC address: you need to fill these out
        dst_mac[0] = pinglist[count].mac_addr[0];
        dst_mac[1] = pinglist[count].mac_addr[1];
        dst_mac[2] = pinglist[count].mac_addr[2];
        dst_mac[3] = pinglist[count].mac_addr[3];
        dst_mac[4] = pinglist[count].mac_addr[4];
        dst_mac[5] = pinglist[count].mac_addr[5];
        dst_mac[6] = 0x00;
        dst_mac[7] = 0x00;
        ...
        if ((status = inet_pton (AF_INET, pinglist[count].pingip, &(send_iphdr->ip_dst))) != 1) {
            fprintf (stderr, "inet_pton() failed.\nError message: %s", strerror (status));
            exit (EXIT_FAILURE);
        }
        ...
        send_icmphdr->icmp_seq =pinglist[count].nsent;
        pinglist[count].nsent++;
...
int main(int argc, char const *argv[])
{
    ...
    while(1)
    {
            ...
            if(ntohs(rt_recv_hdr->id) == IDENTIFICATION)
            {
                ...
                if(rt_recv_payload[1] == (recvlen-6)){
                    ...
                    for(l=0;l<10;l++)
                    {
                        ...
                        if(strcmp(pinglist[l].pingip,previousip) == 0)
                        {
                            flag=1;
                            break;
                        }
                    ...
                    if(flag==1)
                    {
                        printf("\n ping already processed from this node to previous node \n");
        ...
        if (FD_ISSET(pg, &rset))
        {
            ...
            process_pingreply(pgbuff, recvbytes,&tval,pgaddr);
